...
Профили принадлежат этому аккаунту. Миграция `009_admin_card_definitions` назначает `nikita` администратором и добавляет правила настраиваемых Recap-карточек. Остальные аккаунты по умолчанию не имеют административных прав. Seed предназначен для демо/MVP. В production его следует вынести из набора production-миграций или заменить подготовкой демонстрационной среды.
