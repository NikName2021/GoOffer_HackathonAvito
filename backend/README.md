...
Backend персональных «Итогов года»: cookie-аутентификация, CRUD профилей, детерминированная генерация recap, безопасный экспорт, миссии следующего года и Prometheus-метрики. Сервер написан на Go поверх стандартного `net/http`; данные хранятся в PostgreSQL, Redis используется только как кэш готовых итогов.
...
На чистой базе миграция `002_seed_test_profiles` создаёт четыре демонстрационных профиля, а `004_auth` — аккаунт:
...
Профили принадлежат этому аккаунту. Seed предназначен для демо/MVP. В production его следует вынести из набора production-миграций или заменить подготовкой демонстрационной среды.
