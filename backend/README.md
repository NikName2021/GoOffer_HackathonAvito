...
Профили принадлежат этому аккаунту. Миграция `009_admin_card_definitions` назначает `nikita` администратором и добавляет правила настраиваемых Recap-карточек. Миграция `010_enrich_seed_profiles` расширяет набор до шести реалистичных персон: у каждой не менее шести просмотренных и трёх собственных объявлений, а восемь сделок связаны между профилями одинаковыми `adId`, ценой и датой покупки/продажи. Миграция `011_add_seed_activity_and_images` доводит историю каждого профиля минимум до 50 item-level событий и добавляет изображения ко всем просмотренным и собственным объявлениям. Миграция `012_achievement_definitions` переносит шесть встроенных ачивок и правила их получения в PostgreSQL. Остальные аккаунты по умолчанию не имеют административных прав. Seed предназначен для демо/MVP. В production его следует вынести из набора production-миграций или заменить подготовкой демонстрационной среды.
