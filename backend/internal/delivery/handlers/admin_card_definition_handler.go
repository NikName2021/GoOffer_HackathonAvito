...
type AdminCardDefinitionService interface {
	Create(ctx context.Context, adminID uuid.UUID, definition *domain.CardDefinition) (*domain.CardDefinition, error)
	List(ctx context.Context) ([]domain.CardDefinition, error)
	Update(ctx context.Context, id uuid.UUID, definition *domain.CardDefinition) (*domain.CardDefinition, error)
...
func (h *AdminCardDefinitionHandler) Register(mux *http.ServeMux) {
	...
	mux.Handle("GET /api/admin/card-definitions/options", adminOnly(h.Options))
	mux.Handle("GET /api/admin/card-definitions", adminOnly(h.List))
	mux.Handle("POST /api/admin/card-definitions", adminOnly(h.Create))
	mux.Handle("PUT /api/admin/card-definitions/{id}", adminOnly(h.Update))
	mux.Handle("DELETE /api/admin/card-definitions/{id}", adminOnly(h.Delete))
}
...
func (h *AdminCardDefinitionHandler) Update(w http.ResponseWriter, r *http.Request) {
	id, ok := parseCardDefinitionID(w, r)
	if !ok {
		return
	}
	request, err := decodeCreateCardDefinitionRequest(w, r)
	...
	definition := request.cardDefinition()
	updated, err := h.service.Update(r.Context(), id, &definition)
	if err != nil {
		h.writeError(w, r, err)
		return
	}
...
func parseCardDefinitionID(w http.ResponseWriter, r *http.Request) (uuid.UUID, bool) {
...
func (request CreateCardDefinitionRequest) cardDefinition() domain.CardDefinition {
	...
	if definition.Layout == "" {
		...
		if definition.Kind == domain.CardKindHighlight {
			definition.Layout = "hero"
		}
	}
	if definition.Theme == "" {
