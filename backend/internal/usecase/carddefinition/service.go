...
func (s *Service) Create(
	...
	if adminID == uuid.Nil {
		return nil, invalid("administrator ID is required")
	}
...
func (s *Service) Update(
	...
	if id == uuid.Nil {
		return nil, invalid("card definition ID is required")
	}
	normalize(definition)
	if err := validate(definition); err != nil {
		return nil, err
	...
	if err := s.repository.Update(ctx, definition); err != nil {
		return nil, fmt.Errorf("update card definition: %w", err)
	}
	return definition, nil
}
...
func (s *Service) Delete(ctx context.Context, id uuid.UUID) error {
	if id == uuid.Nil {
		return invalid("card definition ID is required")
	}
