...
import type { CardDefinition, CardDefinitionRequest } from '@/types/cardDefinition.type'
...
export function RecapSettingsPage() {
  ...
  async function save(definition: CardDefinitionRequest) {
    if (editing) await updateMutation.mutateAsync({ definition, id: editing.id })
    else await createMutation.mutateAsync(definition)
  }

  const formError = (editing ? updateMutation.error : createMutation.error) as Error | null
        ...
        <div className="mx-auto max-w-[1120px]">
          <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-[-0.03em] sm:text-4xl">Настройка итогов года</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[#6f7377]">
                Создавайте персональные карточки, выбирайте метрику и условия их появления в следующих итогах.
              </p>
            </div>
            <Button
          ...
          {definitionsQuery.isPending && <SettingsNotice text="Загружаем настройки…" />}
          {definitionsQuery.isError && (
            <SettingsNotice action={() => void definitionsQuery.refetch()} text={definitionsQuery.error.message} />
          )}
...
function SettingsNotice({ action, text }: { action?: () => void; text: string }) {
  return (
