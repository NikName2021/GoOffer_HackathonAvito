...
export function ViewsFormSection({ onAdd, onChange, onRemove, views }: ViewsFormSectionProps) {
  return (
    <section>
      <header className="mb-5 flex items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-bold text-[#1f1f1f]">Все просмотры</h3>
          <p className="mt-1 text-xs text-[#8a8d91]">Каждый элемент описывает просмотренное объявление.</p>
        </div>
        <button
      ...
      {views.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-[#dfe1e3] px-5 py-12 text-center">
          ...
          <p className="mt-1 text-xs text-[#a1a4a7]">Добавьте первое просмотренное объявление.</p>
        </div>
      ) : (
