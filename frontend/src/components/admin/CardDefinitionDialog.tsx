...
export function CardDefinitionDialog({
  ...
  const [definition, setDefinition] = useState<CardDefinitionRequest>(initialDefinition ?? emptyDefinition)
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto p-5 sm:max-w-4xl sm:p-7">
        <DialogHeader className="pr-10">
          <DialogTitle className="text-2xl font-black text-[#1f1f1f]">
            {initialDefinition ? 'Редактировать карточку' : 'Новая карточка итогов'}
          </DialogTitle>
          <DialogDescription>
            Настройте правило, по которому backend добавит персональную карточку в следующие итоги года.
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={(event) => void submit(event)}>
          <CardDefinitionRuleFields
            ...
            profiles={profiles}
          />
          <CardDefinitionAppearanceFields definition={definition} onChange={setDefinition} options={options} />
