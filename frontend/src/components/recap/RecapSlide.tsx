...
export function RecapSlide({ card, recap }: RecapSlideProps) {
  ...
  return (
    <article className={cn('relative flex h-full min-h-0 overflow-x-hidden overflow-y-auto rounded-[28px] p-5 sm:p-8', theme.surface)}>
      ...
      <div className="relative z-10 flex min-h-0 w-full flex-col">
        ...
        <div className={cn('mt-5 grid items-center gap-5 py-4 sm:gap-8', card.image_url && 'sm:grid-cols-[1fr_240px]')}>
        ...
        {card.cta && (
          <DialogClose
            nativeButton={false}
            render={<Link className="mt-4 inline-flex w-fit items-center gap-2 rounded-2xl bg-[#00aaff] px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#0099e6]" to={getRecapCtaUrl(card.cta, recap.user_id)} />}
          >
            {card.cta.label}
