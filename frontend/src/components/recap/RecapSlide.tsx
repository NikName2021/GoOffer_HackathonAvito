...
export function RecapSlide({ card, recap }: RecapSlideProps) {
  ...
  return (
    <motion.article
      className={cn('relative flex h-full min-h-0 overflow-x-hidden overflow-y-auto rounded-[32px] border border-white/80 shadow-[0_24px_70px_rgba(31,31,31,0.12)]', isChart ? 'p-4' : 'p-5 sm:p-6', theme.surface)}
    ...
    >
      ...
      >
        <motion.span
          animate={reduceMotion ? undefined : { rotate: [0, 5, -4, 0], y: [0, -5, 0] }}
          className={cn('grid shrink-0 place-items-center rounded-2xl border border-white/80 bg-white/80 shadow-lg backdrop-blur-xl', isChart ? 'size-10' : 'size-12 sm:size-14', theme.accent)}
        ...
        <motion.div
          className={cn('grid items-center', isChart ? 'mt-2 gap-3 py-1' : 'mt-5 gap-5 py-4 sm:gap-8', card.image_url && 'sm:grid-cols-[1fr_240px]')}
          variants={{
            hidden: { opacity: 0, y: 24 },
            show: { opacity: 1, transition: { duration: 0.55 }, y: 0 },
          }}
        >
          <div className="min-w-0">
            ...
            <h2 className={cn('mt-2 max-w-2xl text-3xl leading-[0.98] font-black tracking-[-0.055em] text-balance', isChart ? 'sm:text-[36px]' : 'sm:text-[52px]')}>{card.title}</h2>
            {card.value && <p className={cn(isChart ? 'mt-2 text-2xl font-black sm:text-3xl' : 'mt-4 text-2xl font-black sm:text-4xl', theme.accent)}>{card.value}</p>}
            <p className={cn('max-w-2xl text-sm leading-5 text-[#515459] sm:text-base sm:leading-6', isChart ? 'mt-2' : 'mt-4')}>{description}</p>
          </div>

          {card.image_url && (
            <motion.img
              className="order-first h-36 w-full rounded-3xl border border-white/80 object-cover shadow-2xl sm:order-last sm:h-60"
              src={card.image_url}
              alt={card.title}
              whileHover={reduceMotion ? undefined : { rotate: 1.5, scale: 1.045 }}
            />
          )}
        </motion.div>

        <RecapVisualization visualization={card.visualization} />
        ...
        {card.cta && (
          <DialogClose
            ...
            render={
              <Link
                className="group mt-4 inline-flex w-fit items-center gap-2 rounded-2xl bg-gradient-to-r from-[#00aaff] to-[#008ee6] px-5 py-3 text-sm font-bold text-white shadow-[0_12px_30px_rgba(0,170,255,0.28)] transition hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(0,170,255,0.36)]"
                onClick={() => void sendRecapEvent({ event: 'cta_clicked' }, { keepalive: true })}
                to={getRecapCtaUrl(card.cta, recap.user_id)}
              />
            }
          >
            {card.cta.label}
