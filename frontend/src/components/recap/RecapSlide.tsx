  ...
  return (
    ...
    >
      ...
      <motion.div animate="show" className="relative z-10 flex min-h-0 w-full flex-col" initial="hidden" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }}>
        <motion.span animate={reduceMotion ? undefined : { rotate: [0, 5, -4, 0], y: [0, -5, 0] }} className={cn('grid shrink-0 place-items-center rounded-2xl border border-white/80 bg-white/80 shadow-lg backdrop-blur-xl', isChart ? 'size-10' : 'size-12 sm:size-14', theme.accent)} transition={{ duration: 5, repeat: Infinity }}>
          <RecapIcon name={card.presentation.icon} />
        </motion.span>

        <motion.div className={cn('grid items-center', isChart ? 'mt-2 gap-3 py-1' : 'mt-5 gap-5 py-4 sm:gap-8', card.image_url && 'sm:grid-cols-[1fr_240px]')} variants={{ hidden: { opacity: 0, y: 24 }, show: { opacity: 1, transition: { duration: 0.55 }, y: 0 } }}>
