...
import { RecapGiftCelebration } from './RecapGiftCelebration'
import { RecapGiftParcel } from './RecapGiftParcel'
import { RecapGiftProgress } from './RecapGiftProgress'
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-xl flex-col items-center justify-center text-center">
        ...
        >
          <span className="grid size-8 place-items-center overflow-hidden rounded-full bg-[#00c565] text-xs font-black">
            {profile.avatarUrl ? (
              <ImageWithFallback alt="" className="size-full object-cover" src={profile.avatarUrl} />
            ) : (
        ...
        >
          ...
          <RecapGiftCelebration opening={opening} />
        </motion.button>

        <RecapGiftProgress hits={hits} opening={opening} requiredHits={requiredHits} />
      </div>
    </motion.section>
  )
}
