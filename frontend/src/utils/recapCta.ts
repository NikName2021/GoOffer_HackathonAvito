...
function withProfile(path: string, profileId: string, params?: Record<string, string>) {
...
export function getRecapCtaUrl(cta: RecapCardCTA, profileId: string) {
  ...
  switch (cta.action) {
    ...
    case 'open_listing':
      return adId
        ? withProfile(`${PATHS.AVITO}/items/${encodeURIComponent(adId)}`, profileId)
        : withProfile(`${PATHS.AVITO}/recommendations`, profileId)
    case 'open_own_listing':
      return adId
        ? withProfile(`${PATHS.AVITO}/my/items/${encodeURIComponent(adId)}`, profileId)
